...
- Decompile (jadx): from an APK's viewer, decompile its dex to readable Java, one class at a time. The classes are listed and searchable, and each is decompiled on demand so it stays fast and memory-bounded. Only the dex is extracted (after a free-space check), work runs off the UI thread with progress, and all temporary files are cleaned up on exit. Uses an Android-compatible XML security shim since jadx is desktop-oriented.
...
- Vector rendering: when an XML file is an Android vector drawable (`<vector>`), the XML viewer offers a Render action that actually draws the graphic — parsing the paths, groups and literal fill/stroke colours and painting them on a checkerboard with pinch-zoom. Works for vector XML on disk and for vectors decoded out of an APK's resources.
