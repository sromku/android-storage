...
import com.snatik.storage.app.feature.viewer.ApkAnalyzeScreen
...
fun AppNavigation(navTarget: String? = null, onNavConsumed: () -> Unit = {}) {
    ...
    NavDisplay(
        ...
        entryProvider = entryProvider {
            ...
            entry<Route.MediaPager> { route -> com.snatik.storage.app.feature.media.MediaPagerScreen(startId = route.startId, onBack = ::pop, onOpenVideo = ::openPath, onTiled = { path -> push(Route.TiledViewer(path)) }, onDevelop = { path -> push(Route.Develop(path)) }, onEdit = { path -> push(Route.PhotoEdit(path)) }, onEditVideo = { path -> push(Route.VideoStudio(path)) }) }
            ...
            entry<Route.VideoStudio> { route -> com.snatik.storage.app.feature.video.VideoStudioRoute(path = route.path, onBack = ::pop) }
            ...
            entry<Route.IntentDiscover> {
                com.snatik.storage.app.feature.intents.IntentDiscoverScreen(onBack = ::pop, onBuild = { json -> push(Route.IntentBuilder(specJson = json)) })
            }
