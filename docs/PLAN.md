...
- 2026-09-08 (phase 9): External API done. ApiOperations is one gated layer of 16 operations (list_dir, stat, read_file, write_file, disk_usage, apps, app_info, manifest, providers, query_provider, sql, send_intent, logcat, snapshot, diff, shell) over the core modules. ApiService mounts REST at /api/v1/{op} (GET query params or POST JSON) and an MCP endpoint at /mcp on the phase 8 TransferServer, both behind a persistent bearer token and the API-enabled flag. McpServer answers initialize, tools/list and tools/call over JSON-RPC (notifications get 202); tool errors come back as isError results per MCP convention. Two safety gates (privileged = shell tools, destructive = changes) plus an in-memory audit log shown in the API screen. Verified on the Pixel through adb forward: REST list_dir/shell/manifest/write+read, MCP initialize + tools/list (16) + stat/sql/apps, and the destructive gate refusing write on both surfaces. Audit is in-memory (Room deferred). Kill switch = the enable toggle.

- 2026-09-08 (phase 8): Send and receive done. `core/net`: TransferServer on Ktor CIO (receive page with drag and drop, resumable `PUT /api/upload/{name}?offset=`, `POST /api/upload` multipart, `/api/files`, `/api/download` with Range, `/api/info`), guarded by a 6-character pairing code as bearer/X-Code/query; PeerDiscovery over NSD (`_storageapp._tcp`); PeerClient with resume and byte progress. App: Receive screen (URL, QR via ZXing, code, received list, inbox folder), "Send to device…" from the browser selection with peer list and manual host:port. Verified: laptop upload through `adb forward tcp:8484 tcp:8484`, page, listing, ranged download; emulator sent two files to the Pixel through the host (10.0.2.2). The same server hosts the phase 9 API routes through `extraRoutes`. NSD discovery not exercised (devices were on different networks).

- 2026-09-08 (phase 7): Snapshots and recording done. Room arrives with `core/capture` (KSP, generateKotlin). SnapshotRepository walks a tree through the routed file system into `snapshot_files` with size and mtime, optional SHA-256 and optional copies under filesDir/snapshots; `diff` yields added/removed/modified/moved (moves matched by hash) and a Myers line diff (`LineDiff`) renders text changes when copies exist. RecordingEngine runs logcat (shell, `--uid` per package, filter spec), broadcast monitor events, FileObserver watches and stores events in Room; RecordingService keeps it alive as a foreground service and owns the MediaProjection for ScreenRecorder (MediaRecorder H.264). Export writes a zip with events.jsonl, events.txt and the video to Downloads. App: Capture tab with recording card, snapshot and recording lists, snapshot/diff/file-diff/recording screens; browser sort menu gained "Snapshot this folder". Verified on the Pixel: snapshots with copies and hashes, modified/moved detection, line diff, logcat capture through Shizuku, export, 15 s screen recording. Known limit: FileObserver does not see changes other processes make on FUSE-backed shared storage; watch app-private paths instead. Not done: scheduled snapshots.
...
### Phase 5 — Content providers and databases (done)
...
### Phase 7 — Snapshots and recording (done)
...
### Phase 9 — External API and MCP (done)
