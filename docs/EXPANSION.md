...
- **D (done):** Network screen — live per-app connections from /proc/net/tcp+udp (v4/v6) attributed by uid, remote host reverse-DNS, connection state, per-app byte totals from dumpsys netstats. No VPN, no root beyond shell. Reachable from Tools and from an app's Overview.
